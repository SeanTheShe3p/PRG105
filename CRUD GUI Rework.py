...
class AddGUI:
    def __init__(self, master):
        ...
        # Confirm
        self.confirm_label = tk.Label(self.frame3, textvariable=self.confirm)
        # Buttons
...
class ChangeGUI:
    ...
    def edit_person(self):
        name = self.search.get()
        phone = self.entry_phone
        if phone != self.personal_data.get(name):
            phone = self.entry_phone.get()
        else:
            phone = self.personal_data.get(name)
        self.personal_data.pop(name)
        self.personal_data[name] = phone
        output_file = open("personal_data.bat", 'wb')
        pickle.dump(self.personal_data, output_file)
        output_file.close()
        self.confirm.set('successfully updated')
...
class DeleteGUI:
    ...
    def delete_person(self):
        ...
        pickle.dump(self.personal_data, output_file)
        output_file.close()
        self.confirm.set('successfully deleted')
